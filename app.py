...
DESCRIPTION_ENDPOINT = "https://users.roblox.com/v1/description"
...
def check_verification():
    ...
    description = fetch_profile_description()
...
def fetch_profile_description():
    try:
        response = requests.get(DESCRIPTION_ENDPOINT, headers={"accept": "application/json"})
        if response.status_code == 200:
            description = response.json().get('description', '')
            print(f"Debug Info: Fetched Description: {description}")
            return description
        else:
            print(f"Debug Info: Failed to fetch description. Status Code: {response.status_code}")
            return ""
    except requests.exceptions.RequestException as e:
        print(f"Debug Info: Request Exception: {e}")
        return ""
